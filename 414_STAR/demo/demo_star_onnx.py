                ...
                for box, score in zip(boxes_keep, scores_keep):
                    ...
                    result_boxes.append(
                        Box(
                            ...
                            x2=x_max,
                            y2=y_max,
                            cx=abs(x_min + x_max)//2,
                            cy=abs(y_min + y_max)//2,
                            w=abs(x_max - x_min),
                            h=abs(y_max - y_min),
                        )
                    )
...
class STAR(AbstractModel):
    def __init__(
        self,
        *,
        runtime: Optional[str] = 'onnx',
        model_path: Optional[str] = 'star_300w_Nx3x256x256.onnx',
        providers: Optional[List] = None,
    ):
        """
        ...
        self._swap = (0,3,1,2)
        self._mean = np.asarray([127.5, 127.5, 127.5])
        self._std = np.asarray([127.5, 127.5, 127.5])
    ...
    def _preprocess(
        ...
        if len(face_boxes) > 0:
            for face in face_boxes:
                ...
                ratio = 1.0
                face.x1 = max(0, int(cx - w * 0.5 * ratio))
                face.y1 = max(0, int(cy - h * 0.5 * ratio))
                face.x2 = min(int(cx + w * 0.5 * ratio), image_width)
                face.y2 = min(int(cy + h * 0.5 * ratio), image_height)
            ...
            face_images_np = (face_images_np - self._mean) / self._std
    ...
    def _postprocess(
        ...
        if len(landmarks) > 0:
            for landmark, face_box in zip(landmarks, face_boxes):
                landmark[..., 0] = face_box.cx + (face_box.w // 2 * landmark[..., 0])
                landmark[..., 1] = face_box.cy + (face_box.h // 2 * landmark[..., 1])
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default='star_300w_Nx3x256x256.onnx',
    )
    parser.add_argument(
        '-v',
        '--video',
