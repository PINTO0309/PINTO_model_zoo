...
class Box():
    ...
    person_id: int = -1
    track_id: int = -1
    mouth_prob_open: float = -1.0
    mouth_state: int = -1  # -1: Unknown, 0: Closed, 1: Open
    mouth_label: str = ''
...
class VSDLM(AbstractModel):
    def __init__(
        self,
        *,
        runtime: Optional[str] = 'onnx',
        model_path: Optional[str] = 'vsdlm_l.onnx',
        providers: Optional[List] = None,
    ):
        super().__init__(
            runtime=runtime,
    ...
    def __call__(self, image: np.ndarray) -> float:
        if image is None or image.size == 0:
            raise ValueError('Input image for VSDLM is empty.')
    ...
    def _preprocess(
        ...
        if height <= 0 or width <= 0:
            raise ValueError('Invalid target size for VSDLM preprocessing.')
...
def crop_image_with_margin(
    ...
    *,
    ...
    """Extracts a mouth region with the specified pixel margins."""
    if image is None or image.size == 0:
        return None
    h, w = image.shape[:2]
...
def main():
    ...
    parser.add_argument(
        ...
        '--vsdlm_model',
        type=str,
        default='vsdlm_l.onnx',
        help='ONNX file path for the VSDLM mouth classifier.',
    )
    ...
    vsdlm_model_file: str = args.vsdlm_model
    ...
    mouth_classifier = VSDLM(
        runtime='onnx',
        model_path=vsdlm_model_file,
        providers=providers,
    )
    ...
    while True:
        ...
        for box in boxes:
            if box.classid != 19:
                continue
            mouth_crop = crop_image_with_margin(
                image=image,
                box=box,
                margin_top=2,
                margin_bottom=6,
            ...
            if mouth_crop is None or mouth_crop.size == 0:
                continue
            rgb_mouth_crop = cv2.cvtColor(mouth_crop, cv2.COLOR_BGR2RGB)
            try:
                prob_open = mouth_classifier(image=rgb_mouth_crop)
            except Exception:
                continue
            box.mouth_prob_open = prob_open
            box.mouth_state = 1 if prob_open >= 0.5 else 0
            state_text = 'Open' if box.mouth_state == 1 else 'Closed'
            box.mouth_label = state_text
        ...
        for box in boxes:
            ...
            if classid == 0:
                ...
                # Mouth
                color = (0,255,0) if box.mouth_state == 1 else (0,0,255)
            elif classid == 20:
                # Ear
                color = (203,192,255)
            ...
            if classid == 19 and box.mouth_label:
                attr_txt = box.mouth_label
