...
def input_fn_512():
    input_shapes = [1, height_512, width_512, 3]
    yield [np.zeros(input_shapes).astype(np.float32)]
...
params = tf.experimental.tensorrt.ConversionParams(precision_mode='FP32', maximum_cached_engines=1000)
converter = tf.experimental.tensorrt.Converter(input_saved_model_dir='saved_model_{}x{}'.format(height_512, width_512), conversion_params=params)
converter.convert()
converter.build(input_fn=input_fn_512)
converter.save('tensorrt_saved_model_{}x{}_float32'.format(height_512, width_512))

params = tf.experimental.tensorrt.ConversionParams(precision_mode='FP16', maximum_cached_engines=1000)
converter = tf.experimental.tensorrt.Converter(input_saved_model_dir='saved_model_{}x{}'.format(height_512, width_512), conversion_params=params)
converter.convert()
converter.build(input_fn=input_fn_512)
converter.save('tensorrt_saved_model_{}x{}_float16'.format(height_512, width_512))



model = tf.saved_model.load('tensorrt_saved_model_{}x{}_float32'.format(height_512, width_512), tags=[tf.saved_model.SERVING])
infer = model.signatures[tf.saved_model.DEFAULT_SERVING_SIGNATURE_DEF_KEY]
infer.inputs[0].shape
x = np.random.uniform(size=(1,height_512,width_512,3)).astype(np.float32)
...
model = tf.saved_model.load('tensorrt_saved_model_{}x{}_float16'.format(height_512, width_512), tags=[tf.saved_model.SERVING])
infer = model.signatures[tf.saved_model.DEFAULT_SERVING_SIGNATURE_DEF_KEY]
infer.inputs[0].shape
x = np.random.uniform(size=(1,height_512,width_512,3)).astype(np.float32)
start = time.perf_counter()
