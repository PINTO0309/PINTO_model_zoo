...
class Box():
    ...
    body_label: str = ''
    head_prob_sunglasses: float = -1.0
    head_state: int = -1  # -1: Unknown, 0: no_sunglasses, 1: sunglasses
    head_label: str = ''
...
class SGC(AbstractModel):
    def __init__(
        self,
        *,
        runtime: Optional[str] = 'onnx',
        model_path: Optional[str] = 'sgc_l_48x48.onnx',
        providers: Optional[List] = None,
    ):
        super().__init__(
            runtime=runtime,
    ...
    def __call__(self, image: np.ndarray) -> float:
        if image is None or image.size == 0:
            raise ValueError('Input image for SGC is empty.')
        ...
        prob_sunglasses = float(np.squeeze(outputs[0]))
        return float(np.clip(prob_sunglasses, 0.0, 1.0))
    ...
    def _preprocess(
        ...
        if height <= 0 or width <= 0:
            raise ValueError('Invalid target size for SGC preprocessing.')
...
def main():
    ...
    parser.add_argument(
        ...
        '--sgc_model',
        type=str,
        default='sgc_l_48x48.onnx',
        help='ONNX file path for the SGC sunglasses classifier.',
    )
    ...
    sgc_model_file: str = args.sgc_model
    ...
    while True:
        ...
        for box in head_boxes:
            ...
            try:
                prob_sunglasses = sgc_classifier(image=rgb_crop)
            except Exception:
                continue
        ...
        for box in boxes:
            ...
            attr_color = SUNGLASSES_COLOR if sunglasses_label_active else color
            if attr_txt == '':
                continue
            cv2.putText(
                debug_image,
