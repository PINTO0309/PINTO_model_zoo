...
converter.save('tensorrt_saved_model_mobilenetv2_fsd2018_41cls_float32')
...
converter.convert()
converter.build(input_fn=input_fn)
converter.save('tensorrt_saved_model_mobilenetv2_fsd2018_41cls_float16')



model = tf.saved_model.load('tensorrt_saved_model_mobilenetv2_fsd2018_41cls_float16', tags=[tf.saved_model.SERVING])
infer = model.signatures[tf.saved_model.DEFAULT_SERVING_SIGNATURE_DEF_KEY]
infer.inputs[0].shape
x = np.random.uniform(size=(1, height, width, 1)).astype(np.float32)
