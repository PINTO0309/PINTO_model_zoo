    ...
    def __init__(
        ...
        *,
        ...
        if self._runtime == 'onnx':
            ...
            if onnx_graph.graph.node[0].op_type == "Shape":
                first_resize_op: List[onnx.ValueInfoProto] = [i for i in onnx_graph.graph.value_info if i.name == "/Resize_output_0"]
                if first_resize_op:
...
class RTDETRv2(AbstractModel):
    def __init__(
        self,
        *,
        runtime: Optional[str] = 'onnx',
        model_path: Optional[str] = 'rtdetrv2_r18vd_120e_wholebody25_1250query_n_batch.onnx',
        obj_class_score_th: Optional[float] = 0.35,
        attr_class_score_th: Optional[float] = 0.70,
        ...
        runtime: Optional[str]
            Runtime for RTDETRv2. Default: onnx

        model_path: Optional[str]
            ONNX/TFLite file path for RTDETRv2
        ...
        self.mean: np.ndarray = np.asarray([0.485, 0.456, 0.406], dtype=np.float32).reshape([3,1,1]) # Not used in RTDETRv2
        self.std: np.ndarray = np.asarray([0.229, 0.224, 0.225], dtype=np.float32).reshape([3,1,1]) # Not used in RTDETRv2
...
def main():
    ...
    parser.add_argument(
        ...
        '--model',
        type=str,
        default='rtdetrv2_r18vd_120e_wholebody25_1250query_n_batch.onnx',
        help='ONNX/TFLite file path for RTDETRv2.',
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.65,
        help='Object score threshold. Default: 0.65',
    )
    parser.add_argument(
        '-ast',
        ...
        type=float,
        default=0.70,
        help='Attribute score threshold. Default: 0.70',
    )
    parser.add_argument(
        '-dvw',
    ...
    model = RTDETRv2(
        ...
        obj_class_score_th=float(args.object_score_threshold),
        attr_class_score_th=float(args.attribute_score_threshold),
        providers=providers,
    )
