...
class GazelleHeatmapONNX(AbstractModel):
    ...
    def __init__(
        ...
        if not model_path:
            print(Color.YELLOW("INFO: Gazelle ONNX model path not provided. Heatmap rendering disabled."))
            return
        if not os.path.exists(model_path):
            print(Color.RED(f"WARNING: Gazelle ONNX model not found: {model_path}. Heatmap rendering disabled."))
            return

        try:
            import onnxruntime  # type: ignore  # noqa: F401
        except ImportError:
            print(Color.RED("WARNING: onnxruntime is required for Gazelle heatmap inference but is not installed."))
            return

        runtime = runtime or 'onnx'
        if runtime != 'onnx':
            print(Color.YELLOW(f"INFO: Gazelle heatmap only supports ONNXRuntime. Forcing runtime to onnx (was {runtime})."))
            runtime = 'onnx'
        ...
        print(Color.GREEN("Enabled Gazelle heatmap inference session:"))
        pprint({
            "model": model_path,
    ...
    def __call__(
        ...
        *,
        ...
        image_dtype = self._input_dtypes[0] if len(self._input_dtypes) > 0 else np.float32
        bbox_dtype = self._input_dtypes[1] if len(self._input_dtypes) > 1 else np.float32
    ...
    def _preprocess(
        ...
        *,
        ...
        if image.ndim != 3:
            raise ValueError("GazelleHeatmapONNX expects an image with shape [H, W, C].")
    ...
    def _postprocess(
        ...
        *,
        ...
        # Heatmap inference does not return Box structures.
        return []
...
def main():
    ...
    def check_alpha(value):
        ...
        if fvalue < 0.0 or fvalue > 1.0:
            raise ArgumentTypeError(f"Invalid Value: {fvalue}. Please specify a float in the range [0.0, 1.0].")
        return fvalue
    ...
    parser.add_argument(
        ...
        '--model',
        type=str,
        default='deimv2_hgnetv2_pico_wholebody34_340query_n_batch_640x640.onnx',
        help='ONNX/TFLite file path for DEIMv2.',
    )
    parser.add_argument(
        '--gazelle_model',
        type=str,
        default='gazelle_dinov3_vit_tiny_1x3x640x640_1xNx4.onnx',
        help='Gazelle gaze estimation ONNX file path.',
    )
    ...
    parser.add_argument(
        '--gazelle_heatmap_alpha',
        type=check_alpha,
        default=0.35,
        help='Alpha blending ratio for Gazelle heatmap overlay. Range: 0.0-1.0. Default: 0.35',
    )
    parser.add_argument(
        '--disable_gazelle_heatmap',
        action='store_true',
        help='Disable Gazelle gaze heatmap rendering.',
    )
    ...
    gazelle_model_path: str = args.gazelle_model
    gazelle_heatmap_alpha: float = args.gazelle_heatmap_alpha
    enable_gazelle_heatmap: bool = not args.disable_gazelle_heatmap
    ...
    gazelle_runner: Optional[GazelleHeatmapONNX] = None
    if enable_gazelle_heatmap:
        gazelle_runner = GazelleHeatmapONNX(
            model_path=gazelle_model_path,
            providers=providers,
        )
        if not gazelle_runner.is_ready():
            enable_gazelle_heatmap = False
            gazelle_runner = None
    ...
    while True:
        ...
        if enable_gazelle_heatmap and gazelle_runner is not None:
            head_boxes = [box for box in boxes if box.classid == 7]
            gaze_heatmaps, _ = gazelle_runner.run(image, head_boxes)
            if gaze_heatmaps is not None:
                debug_image = overlay_gaze_heatmaps(
                    debug_image,
                    gaze_heatmaps,
                    alpha=gazelle_heatmap_alpha,
                )
