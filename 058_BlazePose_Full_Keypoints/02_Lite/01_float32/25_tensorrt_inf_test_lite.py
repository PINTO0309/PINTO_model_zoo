...
converter = tf.experimental.tensorrt.Converter(input_saved_model_dir='saved_model_lite_pose_detection', conversion_params=params)
converter.convert()
converter.build(input_fn=input_fn_detection)
converter.save('tensorrt_saved_model_lite_pose_detection_float32')
...
converter.convert()
converter.build(input_fn=input_fn_detection)
converter.save('tensorrt_saved_model_lite_pose_detection_float16')


params = tf.experimental.tensorrt.ConversionParams(precision_mode='FP32', maximum_cached_engines=1000)
converter = tf.experimental.tensorrt.Converter(input_saved_model_dir='saved_model_lite_pose_landmark_39kp', conversion_params=params)
converter.convert()
converter.build(input_fn=input_fn_landmark)
converter.save('tensorrt_saved_model_pose_landmark_lite_body_float32')
...
converter.convert()
converter.build(input_fn=input_fn_landmark)
converter.save('tensorrt_saved_model_pose_landmark_lite_body_float16')




model = tf.saved_model.load('tensorrt_saved_model_lite_pose_detection_float32', tags=[tf.saved_model.SERVING])
