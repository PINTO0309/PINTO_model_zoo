...
OBJECT_CLASS_IDS = {0, 5, 6, 7, 16, 17, 18, 19, 20, 26, 27, 28, 33}
ATTRIBUTE_CLASS_IDS = {1, 2, 3, 4, 8, 9, 10, 11, 12, 13, 14, 15}
KEYPOINT_CLASS_IDS = {21, 22, 23, 24, 25, 29, 30, 31, 32}
YOLOMIT28_TO_LEGACY_CLASS_ID = {
    0: 0,    # body
    1: 1,    # adult
    2: 2,    # child
    3: 3,    # male
    4: 4,    # female
    5: 5,    # body_with_wheelchair
    6: 6,    # body_with_crutches
    7: 7,    # head
    8: 8,    # front
    9: 9,    # right-front
    10: 10,  # right-side
...
WEARING_HAT_LABEL = 'Wearing-Hat'
NO_WEARING_HAT_LABEL = 'No-Wearing-Hat'
WEARING_HAT_COLOR = (0, 210, 0)
WEARING_HAT_SCORE_THRESHOLD = 0.50
...
class Box():
    ...
    body_label: str = ''
    head_prob_wearing_hat: float = -1.0
    head_state: int = -1  # -1: Unknown, 0: no_wearing_hat, 1: wearing_hat
    head_label: str = ''
...
class WholeBodyDetector(AbstractModel):
    def __init__(
        ...
        *,
        ...
        model_path: Optional[str]
            ONNX/TFLite file path for the YOLO whole-body object detector.
    ...
    def __call__(
        ...
        result_boxes: List[Box]
            Predicted boxes: [legacy classid, score, x1, y1, x2, y2, cx, cy, attributes, is_used=False]
        """
    ...
    def _decode_yolomit28_output(
        ...
        *,
        ...
        for idx in keep_indices:
            yolo_class_id = int(best_class_ids[idx])
            classid = YOLOMIT28_TO_LEGACY_CLASS_ID.get(yolo_class_id)
            if classid is None:
                continue
...
class HHC(AbstractModel):
    def __init__(
        self,
        *,
        runtime: Optional[str] = 'onnx',
        model_path: Optional[str] = 'hhc_32x32.onnx',
        providers: Optional[List] = None,
    ):
        super().__init__(
            runtime=runtime,
    ...
    def __call__(self, image: np.ndarray) -> float:
        if image is None or image.size == 0:
            raise ValueError('Input image for HHC is empty.')
        ...
        prob_wearing_hat = float(np.squeeze(outputs[0]))
        return float(np.clip(prob_wearing_hat, 0.0, 1.0))
    ...
    def _preprocess(
        ...
        if height <= 0 or width <= 0:
            raise ValueError('Invalid target size for HHC preprocessing.')
...
def main():
    ...
    parser.add_argument(
        ...
        help='ONNX/TFLite file path for the YOLO whole-body object detector.',
    )
    parser.add_argument(
        '-hm',
        '--hhc_model',
        type=str,
        default='hhc_32x32.onnx',
        help='ONNX file path for the HHC hat classifier.',
    )
    ...
    hhc_model_file: str = args.hhc_model
    ...
    while True:
        ...
        for box in head_boxes:
            ...
            try:
                prob_wearing_hat = hhc_classifier(image=rgb_crop)
            except Exception:
                continue
            box.head_prob_wearing_hat = prob_wearing_hat
            box.head_state = 1 if prob_wearing_hat >= WEARING_HAT_SCORE_THRESHOLD else 0
            box.head_label = WEARING_HAT_LABEL if box.head_state == 1 else NO_WEARING_HAT_LABEL
        ...
        for box in boxes:
            ...
            # TrackID text
            wearing_hat_score_drawn_with_trackid = False
            if enable_trackid_overlay and classid == 7 and box.track_id > 0:
                ...
                if box.head_prob_wearing_hat is not None and box.head_prob_wearing_hat >= 0.0:
                    score_label = box.head_label or WEARING_HAT_LABEL
                    score_text = f'{score_label}: {box.head_prob_wearing_hat:.3f}'
                    score_color = WEARING_HAT_COLOR if box.head_state == 1 else color
                    ...
                    wearing_hat_score_drawn_with_trackid = True
            ...
            wearing_hat_label_active = classid in (0, 7) and box.head_state == 1
            if classid in (0, 7):
                if classid == 7 and wearing_hat_score_drawn_with_trackid:
                    attr_txt = ''
                elif box.head_label or (box.head_prob_wearing_hat is not None and box.head_prob_wearing_hat >= 0.0):
                    attr_txt = f'{box.head_label} {box.head_prob_wearing_hat:.3f}' if box.head_label else f'{box.head_prob_wearing_hat:.3f}'
                else:
                    attr_txt = ''

            attr_color = WEARING_HAT_COLOR if wearing_hat_label_active else color
            if attr_txt == '':
                continue
            cv2.putText(
                debug_image,
