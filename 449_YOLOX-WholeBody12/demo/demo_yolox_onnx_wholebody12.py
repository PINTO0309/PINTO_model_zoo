    ...
    def _postprocess(
        ...
        if len(boxes) > 0:
            ...
            if len(boxes_keep) > 0:
                ...
                hand_boxes = [box for box in result_boxes if box.classid == 8]
                left_right_hand_boxes = [box for box in result_boxes if box.classid in [9, 10]]
                self._find_most_relevant_hand(base_objs=hand_boxes, target_objs=left_right_hand_boxes)
                result_boxes = [box for box in result_boxes if box.classid not in [9, 10]]
...
def main():
    ...
    parser.add_argument(
        ...
        '--model',
        type=str,
        default='yolox_x_wholebody_with_wheelchair_post_0090_1x3x384x640.onnx',
