...
PLAIN_LABEL = 'plain'
NOT_PLAIN_LABEL = 'not_plain'
PLAIN_COLOR = (0, 210, 0)
PLAIN_SCORE_THRESHOLD = 0.50
...
class Box():
    ...
    body_label: str = ''
    head_prob_plain: float = -1.0
    head_state: int = -1  # -1: Unknown, 0: not_plain, 1: plain
    head_label: str = ''
...
class WholeBodyDetector(AbstractModel):
    ...
    def _nms(
        ...
        *,
        ...
        # 4. Return the final remaining box.
...
class BPC(AbstractModel):
    def __init__(
        self,
        *,
        runtime: Optional[str] = 'onnx',
        model_path: Optional[str] = 'bpc_48x48.onnx',
        providers: Optional[List] = None,
    ):
        super().__init__(
            runtime=runtime,
    ...
    def __call__(self, image: np.ndarray) -> float:
        if image is None or image.size == 0:
            raise ValueError('Input image for BPC is empty.')
        ...
        prob_plain = float(np.squeeze(outputs[0]))
        return float(np.clip(prob_plain, 0.0, 1.0))
    ...
    def _preprocess(
        ...
        if height <= 0 or width <= 0:
            raise ValueError('Invalid target size for BPC preprocessing.')
...
def main():
    ...
    parser.add_argument(
        ...
        '--bpc_model',
        type=str,
        default='bpc_48x48.onnx',
        help='ONNX file path for the BPC classifier.',
    )
    ...
    parser.add_argument(
        ...
        help=\
            ...
            'Devices using a MicroSD card or similar for main storage can speed up overall processing.',
    )
    parser.add_argument(
        '-dwk',
        '--disable_waitKey',
    ...
    bpc_model_file: str = args.bpc_model
    ...
    while True:
        ...
        for box in head_boxes:
            ...
            try:
                prob_plain = bpc_classifier(image=rgb_crop)
            except Exception:
                continue
            box.head_prob_plain = prob_plain
            box.head_state = 1 if prob_plain >= PLAIN_SCORE_THRESHOLD else 0
            box.head_label = PLAIN_LABEL if box.head_state == 1 else NOT_PLAIN_LABEL
        ...
        for box in boxes:
            ...
            # TrackID text
            plain_score_drawn_with_trackid = False
            if enable_trackid_overlay and classid == 7 and box.track_id > 0:
                ...
                if box.head_prob_plain is not None and box.head_prob_plain >= 0.0:
                    score_label = box.head_label or PLAIN_LABEL
                    score_text = f'{score_label}: {box.head_prob_plain:.3f}'
                    score_color = PLAIN_COLOR if box.head_state == 1 else color
                    ...
                    plain_score_drawn_with_trackid = True
            ...
            plain_label_active = classid in (0, 7) and box.head_state == 1
            if classid in (0, 7):
                if classid == 7 and plain_score_drawn_with_trackid:
                    attr_txt = ''
                elif box.head_label or (box.head_prob_plain is not None and box.head_prob_plain >= 0.0):
                    attr_txt = f'{box.head_label} {box.head_prob_plain:.3f}' if box.head_label else f'{box.head_prob_plain:.3f}'
                else:
                    attr_txt = ''

            attr_color = PLAIN_COLOR if plain_label_active else color
            if attr_txt == '':
                continue
            cv2.putText(
                debug_image,
