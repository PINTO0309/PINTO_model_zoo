...
class BodyStateHistory:
    def __init__(self, long_size: int, short_size: int) -> None:
        ...
        self.last_active_class: int = 0

    def append(self, state: bool, class_id: int) -> None:
        self.long_history.append(state)
        self.short_history.append(state)
        if state and class_id is not None and class_id > 0:
            self.last_active_class = class_id
...
class PUCClassifier(AbstractModel):
    ...
    def __call__(self, image: np.ndarray) -> np.ndarray:
        if image is None or image.size == 0:
            raise ValueError('Input image for PUC is empty.')
...
def main():
    ...
    def update_history_status(history: BodyStateHistory, candidate_class: int = -1) -> None:
        ...
        if state_interval_judgment:
            active_class = history.last_active_class if history.last_active_class > 0 else candidate_class
            if active_class and active_class > 0:
                history.label = PHONE_USAGE_LABELS.get(active_class, f"class_{active_class}")
            else:
                history.label = ''
    ...
    while True:
        ...
        for box in boxes:
            ...
            if probs is None or probs.size == 0:
                box.phone_class = -1
                continue
            probs = np.asarray(probs, dtype=np.float32).clip(0.0, 1.0)
            if probs.ndim == 0:
                probs = np.array([float(probs)], dtype=np.float32)
            box.phone_probs = probs.tolist()
        ...
        for body_box in body_boxes:
            ...
            detection_state = bool(predicted_class and predicted_class > 0)
