...
def convert_graph_def_to_saved_model(export_dir, graph_filepath, inputs, outputs):
  ...
  with tf.compat.v1.Session(graph=tf.Graph()) as session:
    ...
    tf.compat.v1.saved_model.simple_save(
        ...
        #inputs={input_name: session.graph.get_tensor_by_name('{}:0'.format(node.name)) for node in graph_def.node if node.op=='Placeholder'},
...
convert_graph_def_to_saved_model('./saved_model', './FALSR-A_optimization.pb', ['input_image_evaluate_y:0','input_image_evaluate_pbpr:0'], ['test_sr_evaluator_i1_b0_g/target:0'])
