...
class YOLOv9(AbstractModel):
    def __init__(
        ...
        *,
        runtime: Optional[str] = 'onnx',
        model_path: Optional[str] = 'yolov9_e_wholebody15_post_0145_1x3x480x640.onnx',
        obj_class_score_th: Optional[float] = 0.35,
        attr_class_score_th: Optional[float] = 0.70,
            ...
            if len(boxes_keep) > 0:
                ...
                result_boxes = [
                    box for box in result_boxes \
                        if (box.classid in [1, 2] and box.score >= self._attr_class_score_th) or box.classid not in [1, 2]
                ]
                ...
                if not disable_left_and_right_hand_identification_mode:
                    hand_boxes = [box for box in result_boxes if box.classid == 11]
                    left_right_hand_boxes = [box for box in result_boxes if box.classid in [12, 13]]
...
def main():
    ...
    parser.add_argument(
        ...
        '--model',
        type=str,
        default='yolov9_e_wholebody15_post_0145_1x3x480x640.onnx',
    ...
    while True:
        ...
        for box in boxes:
            ...
            if (classid == 0 and not disable_gender_identification_mode) \
                or (classid == 11 and not disable_left_and_right_hand_identification_mode):
