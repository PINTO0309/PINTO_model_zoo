...
class SCRFD:
    def __init__(self, model_file=None, interpreter=None, nms_thresh=0.4):
    ...
    def forward(self, img, thresh):
        ...
        self.interpreter.set_tensor(input_details[0]['index'], blob)
        ...
        fmc = self.fmc
        ###
        output_details = np.array(output_details)
        if len(output_details) == 6:
            reindex = np.array([2, 4, 0, 3, 5, 1])
        if len(output_details) == 9:
            reindex = np.array([4, 6, 1, 3, 0, 7, 8, 5, 2])
        output_details = output_details[reindex]
        output_details = output_details.tolist()
        ###
        for idx, stride in enumerate(self._feat_stride_fpn):
            # If model support batch dim, take first output
            if self.batched:
                scores = self.interpreter.get_tensor(
                    output_details[idx]['index'])[0]
                bbox_preds = self.interpreter.get_tensor(
                    output_details[idx + fmc]['index'])[0]
                bbox_preds = bbox_preds * stride
                if self.use_kps:
                    kps_preds = self.interpreter.get_tensor(
                        output_details[idx + fmc * 2]['index'])[0] * stride
            # If model doesn't support batching take output as is
            else:
                scores = self.interpreter.get_tensor(
                    output_details[idx]['index'])
                bbox_preds = self.interpreter.get_tensor(
                    output_details[idx + fmc]['index'])
                bbox_preds = bbox_preds * stride
                if self.use_kps:
                    kps_preds = self.interpreter.get_tensor(
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for _ in range(1):
            bboxes, kpss = detector.detect(frame, 0.5, input_size=(640, 480))
