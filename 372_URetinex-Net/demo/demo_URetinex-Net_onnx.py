...
def run_inference(
    ...
    input_image = cv.resize(
        src=image,
        dsize=(input_shape_1[3], input_shape_1[2]),
    )
    ...
    result = onnx_session.run(
        output_names=None,
        input_feed={
            input_name_1: input_image,
            input_name_2: np.asarray(exposure_ratio, dtype=np.float32).reshape(input_shape_2),
        },
    )
...
def main():
    ...
    parser.add_argument(
        ...
        '--movie_file',
        type=str,
        default='360x640_LLIE.mp4',
    )
    parser.add_argument(
        '-m',
        '--model',
        type=str,
        default='./uretinex_net_360x640.onnx',
    )
    parser.add_argument(
        '-e',
        '--exposure_ratio',
        type=int,
        default=5,
        help='Exposure Ratio. Values in the range of 3 to 5 are recommended. Default: 5'
    )
    ...
    input_name_1 = model_inputs[0].name
    input_shape_1 = model_inputs[0].shape
    input_name_2 = model_inputs[1].name
    input_shape_2 = model_inputs[1].shape
    ...
    while True:
        ...
        debug_image_width = debug_image.shape[1]
        cv.putText(
            output_image,
            f"Exposure Ratio : {exposure_ratio}",
            (debug_image_width-230, 30),
            cv.FONT_HERSHEY_SIMPLEX,
            0.7,
            (255, 255, 255),
            2,
