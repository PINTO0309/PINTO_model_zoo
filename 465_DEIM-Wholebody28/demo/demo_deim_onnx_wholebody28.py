...
class DEIM(AbstractModel):
    def __init__(
        self,
        *,
        runtime: Optional[str] = 'onnx',
        model_path: Optional[str] = 'deim_hgnetv2_x_wholebody28_ft_1250query_n_batch.onnx',
        obj_class_score_th: Optional[float] = 0.35,
        attr_class_score_th: Optional[float] = 0.70,
        ...
        self.mean: np.ndarray = np.asarray([0.485, 0.456, 0.406], dtype=np.float32).reshape([3,1,1]) # Not used in DEIM
        self.std: np.ndarray = np.asarray([0.229, 0.224, 0.225], dtype=np.float32).reshape([3,1,1]) # Not used in DEIM
...
def main():
    ...
    parser.add_argument(
        ...
        '--model',
        type=str,
        default='deim_hgnetv2_x_wholebody28_ft_1250query_n_batch.onnx',
        help='ONNX/TFLite file path for DEIM.',
    )
