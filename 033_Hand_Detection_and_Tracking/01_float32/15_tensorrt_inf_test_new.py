...
converter = tf.experimental.tensorrt.Converter(input_saved_model_dir='saved_model_hand_landmark_new', conversion_params=params)
converter.convert()
converter.build(input_fn=input_fn)
converter.save('tensorrt_saved_model_hand_landmark_new_float32')

params = tf.experimental.tensorrt.ConversionParams(precision_mode='FP16', maximum_cached_engines=1000)
converter = tf.experimental.tensorrt.Converter(input_saved_model_dir='saved_model_hand_landmark_new', conversion_params=params)
converter.convert()
converter.build(input_fn=input_fn)
converter.save('tensorrt_saved_model_hand_landmark_new_float16')


model = tf.saved_model.load('tensorrt_saved_model_hand_landmark_new_float32', tags=[tf.saved_model.SERVING])
