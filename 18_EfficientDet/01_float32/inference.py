...
class ServingDriver(object):
  ...
  def __init__(self,
    ...
    self.label_id_mapping = parse_label_id_mapping(self.params.get('label_id_mapping', None))
    ...
    self.min_score_thresh = (min_score_thresh if min_score_thresh is not None else anchors.MIN_SCORE_THRESH)
    self.max_boxes_to_draw = (max_boxes_to_draw or anchors.MAX_DETECTIONS_PER_IMAGE)
  ...
  def export(self,
    ...
    if tflite_path:
      ...
      def representative_dataset_gen():
        for i, image in enumerate(raw_test_data):
          image = image[np.newaxis,:,:,:]
          # print(image)
          print('image.shape:', i, image.shape)
          image = image - 127.5
