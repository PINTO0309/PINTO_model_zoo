...
class Box():
    ...
    person_id: int = -1
    track_id: int = -1
    eye_prob_open: float = -1.0
    eye_state: int = -1  # -1: Unknown, 0: Closed, 1: Open
    eye_label: str = ''
...
class OCEC(AbstractModel):
    ...
    def __call__(self, image: np.ndarray) -> float:
        if image is None or image.size == 0:
            raise ValueError('Input image for OCEC is empty.')
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default='ocec_l.onnx',
        help='ONNX file path for the OCEC eye classifier.',
    )
    ...
    ocec_model_file: str = args.ocec_model
