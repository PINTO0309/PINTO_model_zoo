...
def pred_lines(image,
    ...
    output_name03 = onnx_session.get_outputs()[2].name
    result = onnx_session.run([output_name01, output_name02, output_name03],
                              {input_name: batch_image})
...
def pred_squares(
    ...
    max_acc_map = tf.keras.layers.MaxPool2D(pool_size=(5, 5),
                                            strides=1,
                                            padding='same')(acc_map)
    acc_map = acc_map * tf.cast(tf.math.equal(acc_map, max_acc_map),
                                tf.float32)
