...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        box_cls = torch.cat([xi.view(shape[0], self.no, -1) for xi in x], 2) # onnxruntime-web
        cut_ind = self.reg_max * 4 # onnxruntime-web
        box = box_cls[:, :cut_ind, :] # onnxruntime-web
        cls = box_cls[:, cut_ind:, :] # onnxruntime-web
...
class DualDetect(nn.Module):
    ...
    def forward(self, x):
        ...
        box_cls = torch.cat([di.view(shape[0], self.no, -1) for di in d1], 2) # onnxruntime-web
        cut_ind = self.reg_max * 4 # onnxruntime-web
        box = box_cls[:, :cut_ind, :] # onnxruntime-web
        cls = box_cls[:, cut_ind:, :] # onnxruntime-web
        dbox = dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
        # box2, cls2 = torch.cat([di.view(shape[0], self.no, -1) for di in d2], 2).split((self.reg_max * 4, self.nc), 1)
        box_cls2 = torch.cat([di.view(shape[0], self.no, -1) for di in d2], 2) # onnxruntime-web
        cut_ind2 = self.reg_max * 4 # onnxruntime-web
        box2 = box_cls2[:, :cut_ind2, :] # onnxruntime-web
        cls2 = box_cls2[:, cut_ind2:, :] # onnxruntime-web
