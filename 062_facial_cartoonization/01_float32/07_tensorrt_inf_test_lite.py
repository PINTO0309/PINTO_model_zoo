...
converter.save('tensorrt_saved_model_facial_cartoonization_256x256_float32')

params = tf.experimental.tensorrt.ConversionParams(precision_mode='FP16', maximum_cached_engines=1000)
converter = tf.experimental.tensorrt.Converter(input_saved_model_dir='saved_model_256x256', conversion_params=params)
converter.convert()
converter.build(input_fn=input_fn_256x256)
converter.save('tensorrt_saved_model_facial_cartoonization_256x256_float16')


model = tf.saved_model.load('tensorrt_saved_model_facial_cartoonization_256x256_float16', tags=[tf.saved_model.SERVING])
