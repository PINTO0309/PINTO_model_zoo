...
def input_fn_480x640():
    input_shapes = [1, 448, 448, 3]
    yield [np.zeros(input_shapes).astype(np.float32),np.zeros(input_shapes).astype(np.float32),np.zeros(input_shapes).astype(np.float32)]

params = tf.experimental.tensorrt.ConversionParams(precision_mode='FP32', maximum_cached_engines=1000)
converter = tf.experimental.tensorrt.Converter(input_saved_model_dir='saved_model_448x448', conversion_params=params)
converter.convert()
converter.build(input_fn=input_fn_480x640)
converter.save('tensorrt_saved_model_448x448_float32')

params = tf.experimental.tensorrt.ConversionParams(precision_mode='FP16', maximum_cached_engines=1000)
converter = tf.experimental.tensorrt.Converter(input_saved_model_dir='saved_model_448x448', conversion_params=params)
converter.convert()
converter.build(input_fn=input_fn_480x640)
converter.save('tensorrt_saved_model_448x448_float16')


# model = tf.saved_model.load('tensorrt_saved_model_448x448_float16', tags=[tf.saved_model.SERVING])
# infer = model.signatures[tf.saved_model.DEFAULT_SERVING_SIGNATURE_DEF_KEY]
# x = np.random.uniform(size=(1, 448, 448, 3)).astype(np.float32)
