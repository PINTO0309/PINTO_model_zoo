...
def run_inference(interpreter, input_size, image):
    ...
    result.append(interpreter.get_tensor(
        output_details[0]['index']))  # [  1, 180, 320,   3]
    result.append(interpreter.get_tensor(
        output_details[1]['index']))  # [  1,  35, 80,   3]
    result.append(interpreter.get_tensor(
        output_details[2]['index']))  # [  1,  90, 160,   3]
...
def main():
    ...
    while True:
        ...
        cv.imshow('MIMO UNet Input', debug_image)
        cv.imshow('MIMO UNet Output', output_image)
