...
class FaceBoxes(AbstractModel):
    def __init__(
        self,
        *,
        runtime: Optional[str] = 'onnx',
        model_path: Optional[str] = 'faceboxes_post_1x3x480x640.onnx',
        class_score_th: Optional[float] = 0.35,
        ...
        runtime: Optional[str]
            Runtime for FaceBoxes. Default: onnx

        model_path: Optional[str]
            ONNX/TFLite file path for FaceBoxes
    ...
    def _preprocess(
        ...
        resized_image: np.ndarray = resized_image[..., ::-1]
        ...
        if len(boxes) > 0:
            ...
            if len(boxes_keep) > 0:
                for box, score in zip(boxes_keep, scores_keep):
                    x_min = int(max(0, box[3]) * image_width)
                    y_min = int(max(0, box[4]) * image_height)
                    x_max = int(min(box[5], self._input_shapes[0][self._w_index]) * image_width)
                    y_max = int(min(box[6], self._input_shapes[0][self._h_index]) * image_height)
                    result_boxes.append(
                        Box(
                            classid=int(box[1]),
                            score=float(score),
...
def main():
    ...
    parser.add_argument(
        ...
        '--model',
        type=str,
        default='faceboxes_post_1x3x480x640.onnx',
        help='ONNX/TFLite file path for FaceBoxes.',
    )
    parser.add_argument(
        '-v',
        '--video',
