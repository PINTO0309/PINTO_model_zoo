...
class FairFace(AbstractModel):
    def __init__(
        self,
        *,
        runtime: Optional[str] = 'onnx',
        model_path: Optional[str] = 'attributes_fairface_Nx3x224x224.onnx',
        providers: Optional[List] = None,
    ):
        """
        ...
        if meta_datas:
            ...
            self.labels: List[str] = ast.literal_eval(meta_datas.get('labels', ['0','1']))
    ...
    def __call__(
        ...
        outputs = super().__call__(input_datas=[inferece_image])
        # [N, 7], [N, 2], [N, 9]
        raceids, genderids, ageids = outputs
        # [[race, gender, age], [race, gender, age], ...]
        result_labels = self._postprocess(raceids=raceids, genderids=genderids, ageids=ageids)
    ...
    def _postprocess(
        ...
        *,
        raceids: np.ndarray,
        genderids: np.ndarray,
        ageids: np.ndarray,
    ) -> List[List[str, str, str]]:
        result_labels = [
            [self.race_labels[raceid], self.gender_labels[genderid], self.age_labels[ageid]] \
                for raceid, genderid, ageid in zip(raceids, genderids, ageids)
        ]
        return result_labels
...
def main():
    ...
    attributes_model = FairFace(
        runtime=runtime,
        model_path=attribute_model_file,
        providers=providers,
    )
            ...
            for box, attribute in zip(face_boxes, attributes):
                ...
                if classid not in [1, 3]:
                    ...
                    cv2.putText(
                        ...
                        (
                            box.x1 if box.x1+50 < debug_image_w else debug_image_w-50,
                            box.y1-10 if box.y1-75 > 0 else 20
                        ),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.7,
                        (255, 255, 255),
