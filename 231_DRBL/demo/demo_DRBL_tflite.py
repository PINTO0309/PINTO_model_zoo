...
def run_inference(interpreter, input_size, image):
    ...
    result = []
    result.append(interpreter.get_tensor(
        output_details[0]['index']))  # [ 1, 45, 80,  3]
    result.append(interpreter.get_tensor(
        output_details[1]['index']))  # [  1,  90, 160,   3]
    result.append(interpreter.get_tensor(
        output_details[2]['index']))  # [  1, 180, 320,   3]
...
def main():
    ...
    while True:
        ...
        if key == 27:  # ESC
            break
        cv.imshow('DRBL Stage1 Input', debug_image)
        cv.imshow('DRBL Stage1 Output', output_image)
